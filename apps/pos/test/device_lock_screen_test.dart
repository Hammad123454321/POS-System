...
import 'package:pos_app/src/features/security/device_lock_screen.dart';
import 'package:pos_app/src/features/home/pos_home_controller.dart';
import 'package:pos_app/src/navigation/pos_shell.dart';

void main() {
  testWidgets('shell shows the device lock when reauth is required', (
    tester,
  ) async {
    ...
    final controller = await _build(database, reauth: true);

    await tester.pumpWidget(
      MaterialApp(
        theme: PosTheme.light(),
        home: PosShell(controller: controller),
      ),
    );
    await tester.pumpAndSettle();

    expect(find.byType(DeviceLockScreen), findsOneWidget);
    expect(find.text('Device authorization revoked'), findsOneWidget);
...
''',
  ...
  final controller = PosHomeController(
    ...
    securityStateStore: _Sec(reauth),
    gateway: _Gw(),
...
class _Sec implements DeviceSecurityStateStore {
  ...
  final bool _reauth;
  @override
  Future<bool> requiresReauth() async => _reauth;
  @override
