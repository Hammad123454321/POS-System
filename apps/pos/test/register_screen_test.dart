...
import 'package:pos_app/src/features/checkout/register_screen.dart';
import 'package:pos_app/src/features/home/pos_home_controller.dart';

Future<PosHomeController> _controllerWithCatalog(PosDatabase database) async {
          ...
          payloadJson: '''
{
  ...
  "config": {
    "items": [
      {"id": "i1", "name": "Latte", "currency": "USD", "effective_price_minor": 450, "sold_out": false, "category_id": "c1", "category_name": "Drinks", "sku": "LAT"},
      {"id": "i2", "name": "Bagel", "currency": "USD", "effective_price_minor": 300, "sold_out": false, "category_id": "c2", "category_name": "Food", "sku": "BGL"}
    ],
...
void main() {
  testWidgets('tapping a product adds it to the cart and updates the total',
      (tester) async {
    ...
    final controller = await _controllerWithCatalog(database);

    await tester.pumpWidget(MaterialApp(
      theme: PosTheme.light(),
      home: Scaffold(body: RegisterScreen(controller: controller)),
    ));
    ...
    expect(controller.quantityOf('i1'), 1);
    expect(controller.cartEstimatedSubtotalMinor, 450);
  });

  testWidgets('category filter narrows the product grid', (tester) async {
    tester.view.physicalSize = const Size(1280, 900);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.reset);
